...
export default function Gallery() {
    ...
    // ⚡ Bolt Optimization: Memoize categories to prevent recreation on every render
    const categories = useMemo(() => ['All', ...new Set(masterGroups.map(g => g.category))], [masterGroups]);

    const filteredGroups = useMemo(() => {
        // ⚡ Bolt Optimization: Hoist invariant string operation outside the loop
        const searchLower = search.toLowerCase();

        return masterGroups.filter(group => {
            // ⚡ Bolt Optimization: Short-circuit evaluation - check cheap exact match before expensive string ops
    ...
    // ⚡ Bolt Optimization: Memoize sort to prevent array allocation and sorting on unassociated state changes
