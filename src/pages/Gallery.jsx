...
export default function Gallery() {
    ...
    // ⚡ Bolt: Wrap expensive category derivation in useMemo to prevent recalculation on every render
    const categories = useMemo(() => ['All', ...new Set(masterGroups.map(g => g.category))], [masterGroups]);

    // ⚡ Bolt: Wrap expensive filtering/sorting in useMemo to prevent recalculation on every render
    const sortedGroups = useMemo(() => {
        const lowerSearch = search.toLowerCase(); // ⚡ Bolt: Hoist toLowerCase outside the loop
        ...
        // ⚡ Bolt: Sort in-place on the new `filtered` array to save memory allocation
