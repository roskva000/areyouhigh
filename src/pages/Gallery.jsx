...
export default function Gallery() {
    ...
    // ⚡ Bolt: Memoize categories generation to avoid recreating array and Set on every render
    const categories = useMemo(() => ['All', ...new Set(masterGroups.map(g => g.category))], [masterGroups]);

    // ⚡ Bolt: Memoize list filtering to avoid expensive array operations on unrelated state changes
    const filteredGroups = useMemo(() => {
        // ⚡ Bolt: Hoist string lowercasing out of the loop and skip inner lowercasing if search is empty
        const searchLower = search.toLowerCase();
        return masterGroups.filter(group => {
            const matchesSearch = !searchLower || group.title.toLowerCase().includes(searchLower) ||
    ...
    // ⚡ Bolt: Memoize expensive array sorting
