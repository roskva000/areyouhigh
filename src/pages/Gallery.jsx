...
export default function Gallery() {
    ...
    // ⚡ Bolt: Memoize category extraction to prevent redundant Set creation and array mapping on every re-render
    // Impact: Avoids unnecessary iterations over masterGroups when search/sort parameters change
    const categories = useMemo(() => {
        return ['All', ...new Set(masterGroups.map(g => g.category))];
    }, [masterGroups]);

    // ⚡ Bolt: Memoize filtering and sorting logic, and hoist search string formatting
    // Impact: Reduces CPU overhead by skipping O(N) list manipulations and O(N*M) redundant string lowercasing operations unless dependencies change
    const sortedGroups = useMemo(() => {
        // Hoist search toLowerCase() outside the loop to avoid redundant string parsing
        ...
        // Mutate the newly allocated 'filtered' array in-place to prevent unnecessary array clones
