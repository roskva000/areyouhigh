...
export default function Gallery() {
    ...
    // ⚡ Bolt: Optimize list filtering by memoizing the result, hoisting the invariant
    // string operation (toLowerCase) outside the loop, and using short-circuit evaluation
    // for the category check before performing expensive string matching.
    const filteredGroups = useMemo(() => {
        const searchLower = search.toLowerCase();
        return masterGroups.filter(group => {
            const matchesCategory = activeCategory === 'All' || group.category === activeCategory;
            if (!matchesCategory) return false; // Short-circuit
            if (!searchLower) return true; // Short-circuit
    ...
    // ⚡ Bolt: Memoize the sorting operation to avoid re-sorting on every render
