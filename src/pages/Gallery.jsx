...
export default function Gallery() {
    ...
    // ⚡ Bolt Optimization: Memoize categories to prevent Set creation on every render
    const categories = useMemo(() => ['All', ...new Set(masterGroups.map(g => g.category))], [masterGroups]);

    // ⚡ Bolt Optimization: Memoize filtered groups, calculate toLowerCase once, and use short-circuit evaluation
    const filteredGroups = useMemo(() => {
        const searchLower = search.toLowerCase();
        return masterGroups.filter(group => {
            // Short-circuit: fast exact match first before expensive string operations
    ...
    // ⚡ Bolt Optimization: Memoize sorted groups to prevent array allocation and sorting on every render
