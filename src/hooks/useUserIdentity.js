...
const getStoredIdentity = () => {
    ...
    // 🛡️ Security: Type-check and length-limit untrusted data from localStorage
    if (typeof storedId === 'string') storedId = storedId.substring(0, 50);
    else storedId = null;

    if (typeof storedNick === 'string') storedNick = storedNick.substring(0, 50);
    else storedNick = null;
