...
const FRAGMENT_SHADER = `
  ...
  void main() {
    ...
    // Recursive symmetry
    float sides = 6.0 + floor(u_intensity * 6.0);
    float ma = mod(a, 2.0 * PI / sides) - PI / sides;
    ...
    for(float i = 0.0; i < 4.0; i++) {
        ...
        float d = length(uv);
        float thickness = 0.002 / abs(d - 0.3);
        col += palette(i * 0.1 + r) * thickness;
    }
    
    col *= smoothstep(1.5, 0.5, r); // Vignette
        ...
        <div className="relative w-screen h-[100dvh] overflow-hidden bg-black selection:bg-transparent text-white">
            {!config ? (
                <ExperienceLobby
                    ...
                    description="Witness the infinite recursion of geometric perfection. A meditative journey through mathematical symmetry."
                ...
                <>
                    ...
                    <canvas ref={canvasRef} className="w-full h-full block filter contrast-125 saturate-[1.2]"></canvas>
