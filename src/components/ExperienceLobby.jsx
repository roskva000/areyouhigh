...
const TabButton = ({ id, activeTab, setActiveTab, label, icon: IconComponent }) => {
    ...
    return (
        <button
            ...
            className={`flex-1 pb-3 pt-2 text-[10px] md:text-xs font-mono uppercase tracking-widest border-b-2 transition-all flex items-center justify-center gap-2 focus-visible:ring-2 focus-visible:ring-accent focus-visible:z-10 focus-visible:outline-none ${activeTab === id
...
const PillButton = ({ active, onClick, children, tooltip, className = '' }) => (
    <Tooltip text={tooltip}>
        <button
            aria-pressed={active}
            onClick={onClick}
            className={`w-full py-3 px-3 rounded-xl border transition-all font-mono text-[9px] md:text-[10px] uppercase tracking-wider flex items-center justify-center gap-2 focus-visible:ring-2 focus-visible:ring-accent focus-visible:outline-none ${active
            ...
            <div className="lobby-content relative w-full max-w-[1200px] bg-zinc-950/90 backdrop-blur-3xl border border-white/10 rounded-[2rem] md:rounded-[2.5rem] p-4 md:p-6 shadow-[0_0_100px_rgba(0,0,0,0.9)] h-[90vh] md:h-[85vh] flex flex-col overflow-hidden">
                ...
                <div className="flex-1 min-h-0 flex flex-col md:flex-row gap-6 md:gap-8 overflow-hidden relative z-0">
                    ...
                    <div className="w-full md:w-2/3 flex flex-col min-h-0 bg-black/20 rounded-2xl border border-white/5">
                        {/* Tabs */}
                        <div role="tablist" aria-label="Customization settings" className="flex border-b border-white/10 shrink-0">
                        ...
                        {/* Content Area */}
                        <div role="tabpanel" id={`tabpanel-${activeTab}`} aria-labelledby={`tab-${activeTab}`} className="flex-1 overflow-y-auto custom-scrollbar p-4 md:p-6">
                            {renderContent()}
                        </div>
                    </div>
