...
## 2024-05-31 - [Untrusted LocalStorage Input]
**Vulnerability:** Values retrieved from `localStorage` (`experience_user_id` and `experience_user_nick`) were used without validation or length limits.
**Learning:** Even client-side storage can be manipulated by users (payload injection) and then sent to backend services, risking storage exhaustion.
**Prevention:** Always type-check and length-limit untrusted client-side inputs immediately upon retrieval before passing them to backend APIs.
