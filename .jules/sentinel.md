...
## 2024-05-28 - [Local Storage Payload Injection]
**Vulnerability:** Untrusted client-side inputs retrieved from `localStorage` (`experience_user_id` and `experience_user_nick`) were used directly in Supabase backend requests without type-checking or length-limiting.
**Learning:** Data stored in `localStorage` can be manipulated by malicious actors or scripts. Trusting it blindly can lead to payload injection or storage exhaustion on the backend.
**Prevention:** Always type-check and length-limit data immediately upon retrieval from `localStorage` before using it in application state or backend requests.
