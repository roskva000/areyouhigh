...
## 2024-06-18 - [Untrusted LocalStorage Data in Backend Requests]
**Vulnerability:** Values retrieved from `localStorage` (like `experience_user_id` and `experience_user_nick`) were used directly without validation, risking payload injection and storage exhaustion if sent to the backend.
**Learning:** Client-side storage can be manipulated by the user and should be treated as untrusted input, especially when used as identifiers in API requests.
**Prevention:** Always type-check and length-limit untrusted client-side inputs immediately upon retrieval from storage before passing them to backend services.
