...
## 2024-05-25 - [Untrusted Client Storage Input Validation]
**Vulnerability:** The application retrieved `experience_user_id` and `experience_user_nick` from `localStorage` without validating or limiting their length.
**Learning:** Client-side storage mechanisms like `localStorage` are untrusted input vectors. Data retrieved from them must be validated before being used in backend requests or database operations to prevent payload injection or storage exhaustion.
**Prevention:** Always validate and limit the length of data retrieved from `localStorage` before using it in application state.
