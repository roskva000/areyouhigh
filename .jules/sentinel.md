...
## 2024-10-24 - Untrusted LocalStorage Data Usage
**Vulnerability:** Retrieved `experience_user_id` and `experience_user_nick` directly from `localStorage` without length limits, passing them into backend database requests.
**Learning:** Client-side storage is an untrusted input vector. Relying on it blindly can lead to payload injection or storage exhaustion attacks when used in database operations.
**Prevention:** Always validate, sanitize, and limit the length of data retrieved from client-side storage mechanisms before utilizing it in application state or backend requests.
