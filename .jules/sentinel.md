...
## 2025-05-24 - [Information Exposure via Supabase Error Objects]
**Vulnerability:** Raw PostgrestError objects returned by the Supabase API were being directly logged via `console.error()` and re-thrown as unhandled promise rejections.
**Learning:** `supabase-js` returns `{ data, error }` instead of throwing. When an application directly logs this error object (e.g. `console.error(error)`) or allows it to bubble up to the `window` scope via an unhandled rejection, it exposes database internals (such as RLS errors, query details, and stack traces) to the client browser console.
**Prevention:** Always parse or replace Supabase error objects with generic, obfuscated error messages (e.g., `console.error('Operation failed')` or `throw new Error('Action failed')`) before passing them down to client-side loggers or state.
