...
## 2024-05-25 - [Untrusted Client-Side Storage Injection]
**Vulnerability:** Retrieving `experience_user_id` and `experience_user_nick` from `localStorage` without validation allowed potential payload injection and storage exhaustion, as these values were directly used in Supabase database queries.
**Learning:** Client-side storage mechanisms like `localStorage` are untrusted input vectors and can be manipulated by users.
