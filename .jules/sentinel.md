...
## 2024-10-24 - Unvalidated LocalStorage Input
**Vulnerability:** Client-side inputs from localStorage (`experience_user_id` and `experience_user_nick`) were used directly in backend Supabase requests without type checking or length limiting.
**Learning:** Even though localStorage is client-side, its contents can be manipulated by malicious actors to inject payloads or cause storage exhaustion/database errors when sent to the backend.
**Prevention:** Always treat localStorage data as untrusted user input. Type-check and enforce strict length limits (e.g., max 50 characters) immediately upon retrieval.
