...
## 2024-07-08 - Redundant Array Cloning and Unmemoized List Manipulations
**Learning:** Found significant performance overhead in React functional components (`Gallery.jsx`) where large lists are filtered and sorted on every render without memoization. Furthermore, chaining `.sort()` after array allocations like `.filter()` with redundant spread syntax (`[...filtered].sort`) causes unnecessary memory allocations and garbage collection.
**Action:** Always wrap expensive list manipulations like `.filter` and `.sort` in `useMemo` when they depend on specific states. Additionally, eliminate redundant array clones when chaining mutating methods after allocations.
