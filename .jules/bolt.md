...
## 2024-05-24 - React Array Mutations
**Learning:** Removing `[...array]` before `array.sort()` on a freshly filtered array inside a React component render function is a risky pattern. While technically safe if `array` is genuinely freshly allocated during that specific render (like `filteredGroups` resulting directly from `.filter()`), it's too subtle. Future refactors could easily memoize `filteredGroups`, turning this safe optimization into a critical React state mutation bug (violating immutability). The tiny performance gain avoids GC on a small array but adds disproportionate risk.
**Action:** Do not remove explicit array copies (e.g., `[...array]`) before mutating operations like `.sort()`, `.reverse()`, or `.splice()` in React components, unless you are 100% certain the array is scoped to a non-React context or is guaranteed to remain an ephemeral calculation.
