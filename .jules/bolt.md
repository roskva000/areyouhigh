...
## 2026-04-21 - Memoization of Sorting and Filtering in React Components
**Learning:** Found that `useMemo` requires manual importing and is missing by default in `Gallery.jsx`. Also, failing to wrap derived state (like filtering and sorting arrays) in `useMemo` causes expensive operations to re-run on every re-render (e.g., when unrelated state like `allVotes` changes), significantly degrading performance.
**Action:** When implementing `useMemo` for performance optimizations, always ensure `useMemo` is explicitly imported from 'react'. Always apply `useMemo` to expensive array operations (filter, sort, map) in functional components, and hoist string operations like `.toLowerCase()` outside loops.
