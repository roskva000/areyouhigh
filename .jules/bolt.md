...
## 2024-05-24 - React useMemo for Array Filtering
**Learning:** Found that array filtering and mapping operations (`filter`, `reduce`), especially those involving string operations like `toLowerCase()` inside loops, run on every render in React and can cause performance degradation.
**Action:** When filtering static arrays or performing complex string manipulations derived from dynamic route parameters, always wrap the operations in a `useMemo` block and hoist invariant calculations (like converting a search string to lowercase) outside the loop to prevent redundant recalculations.
