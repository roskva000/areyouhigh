...
## 2024-03-24 - Memoize Expensive Filtering in Gallery
**Learning:** Re-evaluating derived lists (`filteredGroups`, `sortedGroups`, `categories`) on every render in React blocks the main thread, especially when typing in search inputs, creating noticeable UI lag for data-heavy views.
**Action:** Always wrap heavy list derivations in `useMemo` when they depend on specific state like `search`, and extract string manipulation (like `toLowerCase()`) outside of iteration loops to avoid redundant computations per element.
