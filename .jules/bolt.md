...
## 2024-05-23 - Memoizing Array Derivations with useMemo
**Learning:** In React components like `Gallery.jsx` that manage significant data filtering and sorting (`masterGroups`, `filteredGroups`, `sortedGroups`), recalculating these arrays on every render (even when underlying state hasn't changed) can lead to unnecessary processing overhead. This is especially true if string operations like `toLowerCase()` are done repeatedly inside tight `filter` or `sort` loops.
**Action:** Use `useMemo` to cache the results of expensive array operations, ensuring they only recalculate when their dependencies change. Additionally, hoist string conversion operations (e.g., `search.toLowerCase()`) outside of iteration loops to avoid redundant conversions.
