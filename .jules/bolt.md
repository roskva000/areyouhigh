...
## 2025-04-19 - Gallery filtering and sorting optimizations\n**Learning:** Found that `search.toLowerCase()` was executed inside a filter loop for every single experience mapping over items, causing unnecessary recalculations every frame. Same for derived arrays (categories, sortedGroups, filteredGroups), they were recomputed on every render. \n**Action:** Use `useMemo` to memoize the arrays, and hoist string casing outside of any iterative `.filter` / `.map` / loops.
